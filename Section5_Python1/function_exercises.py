...
def seq_check(nums):
    for i in range(len(nums)-2):
        # index 0, 1, 2, 3, .... n-2, because we need i+2
        if nums[i] == 1 and nums[i+1] == 2 and nums[i+2] == 3:
            return True
    return False


mynums = [2,3,4,5,5,1,2,3,1,3]

print(seq_check(mynums))
...
def compare_len(s1,s2):
    value = abs(len(s1) - len(s2))
    return(value)

print(compare_len('hello','hey'))
...
sum_max_list = [1,2,3,4]
print(sum_or_max(sum_max_list))
